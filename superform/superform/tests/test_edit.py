...
def test_edit_new_post(client):
    ...
    rv = client.post(url, data=dict(titlepost='An edited test post', descriptionpost="A description",
                                    linkurlpost="http://www.test.com", imagepost="image.jpg", datefrompost="2018-07-01",
...
def test_edit_forbidden(client):
    ...
    assert rv.status_code == 200
    rv = client.post(url_post, data=dict(titlepost='An edited test post', descriptionpost="A description",
                                         linkurlpost="http://www.test.com", imagepost="image.jpg",
                                         datefrompost="2018-07-01", dateuntilpost="2018-07-01"))
    # assert rv.status_code != 302
    edited_posts = db.session.query(Post).all()
